# -*- coding: utf-8 -*-
'''
训练情感分析模型
'''

# import the necessary packages
from oldcare.datasets import SimpleDatasetLoader
from oldcare.preprocessing import AspectAwarePreprocessor
from oldcare.preprocessing import ImageToArrayPreprocessor
from oldcare.conv import MiniVGGNet
from oldcare.callbacks import TrainingMonitor
from imutils import paths
from sklearn.model_selection import train_test_split
from keras.optimizers import SGD
from sklearn.metrics import classification_report
from sklearn.preprocessing import LabelEncoder

# 全局变量
dataset_path = '../../dataset'
output_model_path = 'Model/face_expression.hdf5'
output_plot_path = 'plots/face_expression.png'

# 全局常量
TARGET_WIDTH = 28
TARGET_HEIGHT = 28
BATCH_SIZE = 64
EPOCHS = 20
LR_INIT = 0.01
DECAY = LR_INIT / EPOCHS
MOMENTUM = 0.9

# 加载图片
aap = AspectAwarePreprocessor(TARGET_WIDTH, TARGET_HEIGHT)
iap = ImageToArrayPreprocessor()

print("[INFO] loading images...")
class_labels = ["Happy", "Neutral", "Sad"]
imagePaths = []
labels = []

for i, class_label in enumerate(class_labels):
    folder_path = f"{dataset_path}/{class_label}"
    class_image_paths = list(paths.list_images(folder_path))
    imagePaths.extend(class_image_paths)
    labels.extend([i] * len(class_image_paths))


sdl = SimpleDatasetLoader(preprocessors=[aap, iap])
(data, labels) = sdl.load(imagePaths, 500, True)
data = data.astype("float") / 255.0

# Encode labels into numeric form
le = LabelEncoder()
labels = le.fit_transform(labels)

# partition the data into training and testing splits using 80% of
# the data for training and the remaining 20% for testing
(trainX, testX, trainY, testY) = train_test_split(data,
                                                  labels, test_size=0.20, stratify=labels, random_state=42)

# initialize the model
print("[INFO] compiling model...")
num_classes = len(class_labels)
model = MiniVGGNet.build(width=TARGET_WIDTH,
                         height=TARGET_HEIGHT, depth=1, classes=num_classes)
opt = SGD(lr=LR_INIT, decay=DECAY, momentum=MOMENTUM, nesterov=True)
model.compile(loss="sparse_categorical_crossentropy", optimizer=opt,
              metrics=["accuracy"])

# construct the set of callbacks
callbacks = [TrainingMonitor(output_plot_path)]

# train the network
print("[INFO] training network...")
H = model.fit(trainX, trainY, validation_data=(testX, testY),
              batch_size=BATCH_SIZE, epochs=EPOCHS,
              callbacks=callbacks, verbose=1)

# evaluate the network
print("[INFO] evaluating network...")
predictions = model.predict(testX, batch_size=64)
class_labels = [label.split('/')[-1] for label in le.classes_]
print(classification_report(testY,
                            predictions.argmax(axis=1), target_names=class_labels))

# save the model to disk
print("[INFO] serializing network...")
# Save model architecture as JSON
model_json = model.to_json()
with open('../../Model/face_expression.json', 'w') as json_file:
    json_file.write(model_json)

# Save model weights
model.save_weights('Model/face_expression_weights.h5')
